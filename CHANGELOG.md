...
**小改动：橡果换用作者手绘的 16×16 像素画贴图。** 玩法、数值、配方与 1.1.1 完全一致。

### 变更

* **橡果贴图改为手绘像素画。** 此前是从 1254×1254 插画压缩而来（100 色、边缘发糊），
  现在换成作者手绘的 **16×16、39 色**版本，边缘硬、无压缩混色。内容 9×10，留白 L4 R3 T3 B3。
* **橡果酱贴图改用 32×32。** 该贴图源自同一张插画（624 色），在 16×16 下标签细节会被压糊，
  改用 32×32 后标签上的橡果图案、玻璃反光、盖子纹理都能看清。
* 两者画布尺寸不同（16×16 / 32×32），Minecraft 均支持；橡果酱的 16×16 手绘版完成后会另行统一。
* 模组图标同步更新。
