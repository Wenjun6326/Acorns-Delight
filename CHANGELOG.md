...
**小改动：修正橡果酱配方用料 + 贴图缩小一个像素。** 玩法与数值与 1.1.0 一致。

### 修复

* **橡果酱配方多要了 2 个橡果。** 配方图案曾写成 `A A / ACA / A A`，中间一行左右两侧又各有一个橡果，
  实际需要 **6 橡果 + 1 可可豆**，而不是文档中写的 4 个橡果。
  现改为 `A A / " C " / A A`，即**四角放橡果、正中放可可豆**，正好 **4 橡果 + 1 可可豆**。
...
* 在既有的配方测试中新增断言：旧的 6 橡果摆法**必须不再匹配**，确保配方是真的收窄了，而不是碰巧也能合成。
* `All 6 required tests passed :)`（Fabric Loader 0.18.4）
